...
public:
    ...
    void writeNodesInOrderToBinaryFile(ofstream& outFile);
    void readAllNodes(node<T, min_degree> currentNode, vector<node<T, min_degree>>& nodes);

    node<T, min_degree>& getRoot(){return this->root = this->diskReadNo(1);}

    pair<node<T, min_degree>, int> bTreeSearch(node<T, min_degree>& x, const T& k);
...
template <class T, const unsigned int min_degree>
void Btree<T, min_degree>::writeNodesInOrderToBinaryFile(ofstream& outFile) {
    vector<node<T, min_degree>> nodes; // Vetor para armazenar os nós

    // Lê todos os nós da árvore e os armazena no vetor
    readAllNodes(this->root, nodes);

    // Ordena os nós com base na primeira chave de cada nó
    auto cmd = [](const node<T, min_degree>& a, const node<T, min_degree>& b) {
        return a.getKeysConst(0).getValue() < b.getKeysConst(0).getValue();
    };
    sort(nodes.begin(), nodes.end(), cmd);

    // Grava os nós no arquivo binário em ordem crescente
    for (auto& n : nodes) {
        for (int i = 0; i < n.getN(); i++) {
            int keyValue = n.getKeys(i).getValue();
            outFile.write(reinterpret_cast<const char*>(&keyValue), sizeof(int));
        }
    }
}

template <class T, const unsigned int min_degree>
void Btree<T, min_degree>::readAllNodes(node<T, min_degree> currentNode, vector<node<T, min_degree>>& nodes) {
    // Adiciona o nó atual ao vetor
    nodes.push_back(currentNode);

    // Se não for folha, lê recursivamente todos os filhos
    if (!currentNode.getLeaf()) {
        for (int i = 0; i <= currentNode.getN(); i++) {
            long long int indexChild = currentNode.getChildren(i);
            node<T, min_degree> childNode = diskReadNo(indexChild);
            readAllNodes(childNode, nodes);
        }
    }
}
...
template <class T, const unsigned int min_degree>
pair<node<T, min_degree>, int> Btree<T, min_degree>::bTreeSearch(node<T, min_degree>& x, const T& k){
    int i = 0;
    ...
    if((i < x.getN()) && (k == x.getKeys(i))){
        return make_pair(x,i);
            ...
            node<T, min_degree> y;
            y = diskReadNo(x.getChildren(i));  //novo no raiz irá ser o filho x.children[i]
