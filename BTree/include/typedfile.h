...
bool typedFile<T, min_degree>::open(const string name, const string type, const unsigned int ver, ios::openmode openmode)
{
    ...
    else
    {
        ...
        if((type == this->head.getType()) && (ver == this->head.getVersion()) && (ver == min_degree))          //verifica se o arq é da mesma versao e do tipo declarado anteriormente
        {
        ...
        else
        {
            ...
            cout<< "Verifique se o grau minimo é o mesmo valor que a versao, cada arvore b em memoria tem seu proprio grau minimo" <<endl;
            this->close();
        }
